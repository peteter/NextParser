...
namespace Gotoland.NextParserV2
{
    ...
    public partial class Next
    {
        ...
        {
            (_, var partSegment, var rest) = GetParameters(input, offset);
            var part = input.Slice(partSegment).Trim().TrimOne(new[] { '(', ')' });
        ...
        private (Elem, Segment) ParseFunction(ReadOnlySpan<char> nextElement, ReadOnlySpan<char> rest, int offset = 0)
        {
            ...
            var (parameters, partSegment, reminder) = GetParameters(rest, offset);
            var part = rest.Slice(partSegment).Trim(); // Don't need the whitespace
            if (!part.EndsWith(")"))
            ...
            {
                var parameter = Parse(param.text, left: null, param.position.start);
        ...
        /// <summary>
        /// Get the parameters withing a pair of parenthesis.
        /// </summary>
        /// <param name="input">The input span.</param>
        /// <param name="offset">The offset of the input span in the input string.</param>
        /// <returns>A list of start-stop and the text of the parameter.</returns>
        /// <exception cref="BadFormulaException"></exception>
        public (List<((int start, int stop) position, string text)> parameters, Segment parameterPart, Segment rest) GetParameters(ReadOnlySpan<char> input, int offset)
        {
            // OPT TODO: Return just indexes instead? Don't include the string.

            if (input[0] != '(')
                throw new BadFormulaException("The parameter input was missing an opening parenthesis!");

            var list = new List<((int start, int stop), string text)>();
                   ...
                   && (left == 0 || left != right))
            {
                ...
                else if (!insideQuote)
                {
                    ...
                    if (addParam)
                    {
                        ...
                        {
                            list.Add(((start + offset, stop + offset), param));
                        }
            ...
            while (i < input.Length)
            {
                ...
                else if (!haveQuote)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Do we have something in the pipe before the seaprator came?
